...
//     console.warn('⚠️ [LOGIN] Payload inválido. Faltan identifier o password');
//     return null;
//   }

//   // Evitar TypeError: validar identifier antes de usar .includes
...
//   console.log('👤 [LOGIN] Usuario encontrado:', user ? `Sí (${user.email || user.dni})` : 'No');

//   // Función segura para enviar logs sin romper el flujo de autenticación
//   const logAttempt = async (level, message, context = {}) => {
//     try {
//       const logData = {
//         level,
//         message,
//         user: user ? (user.email || user.dni) : identifier,
//         clientIp,
//         context,
//         module: envs.moduleName
//       };
//       // Si usás sendLog, descomenta la línea siguiente y asegura que no tire
//       // await sendLog(logData);
//       console.log('📤 [LOGIN] Log (simulado):', JSON.stringify(logData));
//     } catch (err) {
//       // NO lanzar error desde aquí: el logging no debe impedir el login
//       console.error('❌ [LOGIN] Error enviando log (ignored):', err.message || err);
//     }
//   };
...
//   // Seguridad: asegurar que la contraseña existe en el documento
//   if (!user.password) {
//     console.error('❌ [LOGIN] El usuario no tiene password almacenado (inconsistencia).');
//     await logAttempt('ERROR', 'LOGIN_FAILED - Usuario sin password', { userId: user._id?.toString() });
//     return null;
//   }
...
//   // Función de logging DESHABILITADA
//   const logAttempt = async (level, message, context = {}) => {
//     console.log(`📝 [LOGIN] ${level}: ${message}`, context);
//     // Logging deshabilitado - solo console.log
//   };
